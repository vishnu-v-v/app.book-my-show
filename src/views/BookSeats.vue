    ...
    <b-container class="bv-example-row">
      <b-row>
        <div v-for="(seat, index) in seats" :key="seat.id">
          <button class="btn"
            :class="{ 'btn-primary': seat.selected,
            'btn-secondary': !seat.selected,
            'btn-danger': seat.attributes.reserved }"
            :disabled="seat.attributes.reserved" @click="selectSeats(index)">
  ...
  methods: {
    ...
    bookTickets() {
      ...
      this.$http.secured
        ...
        .then((response) => {
          let success_message = `${response.data.message}\nKrishi Kalyan Cess: ${response.data.krishi_kalyan_cess}\nSwach Bharath Cess: ${response.data.swach_bharath_cess}\nService Tax: ${response.data.service_tax}\nNet Amount: ${response.data.net_amount}\nTotal: ${response.data.total}`
          alert(success_message);
          this.$router.replace("/movies");
        })
        .catch(error => this.setError(error, "Some error occured, please try again later"));
    }
  }
};
</script>
